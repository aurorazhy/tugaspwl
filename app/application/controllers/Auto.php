...
class Auto extends CI_Controller
{
   ...
   public function autobelanja()
   {
      ...
      for ($i = 0; $i < count($nm); $i++) {
         $sql = "INSERT INTO `detail_transaksi`(`id_transaksi`,`nama_barang`, `harga_beli`, `qty`, `harga_jual`, `id_kategori`) VALUES ('$nm[$i]','$hb[$i]','$qty[$i]','$hl[$i]','$id_kat[$i]') ON DUPLICATE KEY UPDATE `nama_barang`= '$nm[$i]' ,`qty`= qty + '$qty[$i]'";
         $this->db->query($sql);
      }
